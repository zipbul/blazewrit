...
| flow-state.json | **안전** | 스텝 사이에만 업데이트 → 마지막 완료 스텝을 가리킴 |
...
| PostToolUse 훅의 Agent 파라미터 접근 | **미검증** | 우회 가능: flow-state.json + 산출물 파일 존재 여부로 판단 |
