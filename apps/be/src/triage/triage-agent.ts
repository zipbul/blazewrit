...
export class TriageAgent {
  ...
  async chat(text: string): Promise<TurnResult> {
    ...
    const options: Options = {
      ...
      allowedTools: [DB_READ_TOOL_FQN, PROPOSE_INTENT_TOOL_FQN, RECORD_FEEDBACK_TOOL_FQN],
    ...
    for await (const message of run({ prompt: text, options })) {
      ...
      if (message.subtype === 'success') {
        return { reply: (message as SDKResultSuccess).result ?? '', intent: captured, feedback };
      }
