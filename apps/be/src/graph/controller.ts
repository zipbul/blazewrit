...
export interface GraphController {
  ...
  /**
   * F4 (3자 리뷰 수정 라운드): stops the periodic timer AND awaits whatever tick() is CURRENTLY
   * in flight (the auto-initial pass, the periodic timer's own last-fired call, or a manual
   * tick() call already running) before resolving. Before this fix, stop() only cleared the
   * FUTURE timer — an in-flight tick from one caller's controller could keep running well past
   * that caller's own test/scope having "stopped" it, and since tick()'s own task scan is GLOBAL
   * (every open task, not scoped to whichever caller started this controller), it could claim and
   * dispatch a job that belongs to a DIFFERENT, unrelated caller's just-created graph — the root
   * cause behind graph-chaos.sim.spec.ts's cross-seed contamination (one seed's leftover
   * controller reconciling into the NEXT seed's task, through the FIRST seed's own now-orphaned
   * app instance/pendingDecisions map). Does not disable future manual tick() calls — only the
   * periodic timer and whatever pass was already running when stop() was called.
   */
  stop(): Promise<void>;
}
...
export function startGraphController(sql: SQL, dispatch: (job: ReconcileJob) => Promise<void>, opts: GraphControllerOpts = {}): GraphController {
  ...
  let inFlight = false;
  // F4: whichever tick() invocation is CURRENTLY running (or, if none is, the last one that ran) —
  // stop() awaits this so it never resolves while a pass is still touching the DB. Seeded resolved
  // so a stop() called before the very first tick ever starts is an immediate no-op, not a hang.
  let lastTickPromise: Promise<TickResult> = Promise.resolve({ expired: [], reconciled: [], wakes: [] });
  ...
  const tick = async (): Promise<TickResult> => {
    ...
    const runTick = async (): Promise<TickResult> => {
      try {
        ...
        // Single-writer round (job-graph.md C1): the crash/restart net for job_events — reconcileTask's
        // own inline call already consumes a task's events immediately on the common path (see its
        // own doc comment), but an event whose fire-and-forget consumer never got to run (process died
        // right after the insert) would otherwise sit unprocessed forever, since this tick's own task
        // scan below only visits tasks with a pending/blocked job — a job stuck 'running' with an
        // unconsumed completion event wouldn't be one of those. Unscoped (every task, not just the
        // ones this tick would otherwise reconcile) closes that gap; run first so every scan below
        // (lease-expiry, wake conditions) sees the post-consumption state, not a stale 'running' one.
        await consumeJobEvents(sql);
  ...
  return {
    tick,
    stop: async () => {
      clearInterval(timer); // B5
      await lastTickPromise; // F4: drain whatever pass is currently in flight before resolving
    },
  };
}
