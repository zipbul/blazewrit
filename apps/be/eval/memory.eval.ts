...
import { recordTurn } from '../src/triage/chat/turns';
import { assembleHistory } from '../src/triage/chat/turn-runner';
...
async function seed(): Promise<Case[]> {
  ...
  await sql`insert into work_items (id, project_id, type, state, title) values (${EVAL_SCOPE}, ${MARK}, 'task', 'in_flow', ${MARK + ' 평가 스레드'})`;
  ...
  await plant(EVAL_SCOPE, `${MARK} 참고: 배포 담당자는 박지훈이야.`, '기억했어 — 배포 담당자는 박지훈.');
  await plant(EVAL_SCOPE, `${MARK} 결제 승인이 자꾸 실패해서 PG사에 문의 넣어뒀어.`, '결제 승인 실패 건, PG사 문의 접수로 기록했어.');
  await plant(EVAL_SCOPE, `${MARK} 스테이징 서버 접속키는 다음주에 로테이션하기로 했어.`, '스테이징 접속키 다음주 로테이션, 확인.');
  await plant(TASK_SCOPE, `${MARK} 이 작업 마감은 8월 14일로 정하자.`, '마감 8월 14일로 기록했어.');
  for (let i = 1; i <= 15; i++) await plant(EVAL_SCOPE, `${MARK} 필러 ${i}`, `${MARK} 필러답 ${i}`);
  ...
  return [
    ...
    },
    {
      ...
      scope: EVAL_SCOPE,
      ask: '스테이징서버 접속키 얘기했던 거 언제 바꾸기로 했지?',
...
async function cleanup() {
  await sql`delete from chat_messages where text like ${MARK + '%'} or scope in (${TASK_SCOPE}, ${EVAL_SCOPE})`;
...
async function main() {
  ...
  for (const c of cases) {
    ...
    for (let r = 0; r < RUNS; r++) {
      const turn = await agent.chat({ request: c.ask, scope: c.scope, history: await assembleHistory(sql, c.scope) });
    ...
    await cleanup(); // even a mid-run LLM failure must not pollute real threads
  }
