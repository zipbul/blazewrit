...
## 스텝 단위 독립 완전성 검토 (더 엄격한 2차 패스 — 20 agent, 2026-05-31)

> 위 A/B/C 매트릭스는 *사용자 기준 부합*("커버/견고/결과")을 봤다. 이 패스는 다른 질문 — **각 스텝이, 다른 스텝과 무관하게, 그 자체로 계약이 완전한가**(① 계약 완전성 ② 경계 ③ 자기실패 처리 ④ garbage-in 견고성). 더 엄격하니 더 가혹: **완전 0 / 중대 8 / 경미 2.** "완벽한 플로우"는 스텝 레벨에서 성립 안 함.

| 스텝 | 판정 | worst hole (그 스텝 자체의) |
|---|:--:|---|
| triage | 🔴 중대 | loop-prevention이 구조적 불완전 — 종단 신호 `ambiguous(escalate)`가 schema(`ambiguous{question}`)에 필드 없음 + cap이 stateless triage가 검증 못 하는 `reclassify_count`에 의존. `complexity_signal`은 *필수인데 생산 활동 없음* |
| ground | 🟠 경미 | `volatile_profile` 선언 위치(`.blazewrit/flows/*.md`)에 실제 필드 없음 + `skipped-with-reason`의 reason carrier 필드 부재. (적대: ED-부재 worst-hole은 *과장* — capture 실패는 unknowns로 매핑됨) |
| investigate | 🟠 경미 | 빈/degenerate Ground(entry_nodes 0) 방어 없음 → V1("issues 빈 list→proceed 강제")이 non-actionable Ground를 **자동 proceed 고무도장** |
| decide | 🔴 중대 | 비강제 mode 결정이 "Investigate 옵션 신호"를 읽는데 **Investigate는 옵션 생산 금지**(계약 모순). N=0 옵션 동작 미정의 + 동시 force 규칙 충돌 시 precedence 없음 |
| spec | 🔴 중대 | 정체성이 Decide(**Design**)에 결박됐는데 Refactor/Migration은 Decide(**Plan**) 뒤에 Spec 실행 → design document/intent card 부재 시 핵심 활동 미정의. `measurement` 필드 포맷 무정의인데 reviewer는 "측정 가능" 요구 |
| test | 🔴 중대 | `status: RED` 하드코딩, GREEN/예상밖 분기 없음 — 그런데 *정상 발생*(재현 안 됨/이미 fix/P0 retroactive는 merge 후 GREEN). 자기 핵심 실패모드 미정의 |
| implement | 🔴 중대 | reviewer 핵심 기준 `deviation_rules`가 **어느 스텝도 생산 안 하는 dangling 참조**. `firebat blockers>0`/`drift≠0`/tool 부재 시 동작 미정의(분기 열고 안 닫음) |
| report | 🔴 중대 | Spike의 GO/NO-GO verdict가 Report 출력 계약에 **자리 없음** — 자신이 서빙하는 6 flow 중 하나의 terminal artifact를 못 냄. severity/priority enum 무정의 |
| verify | 🔴 중대 | **PASS 출력이 미정의** + multi-pass 결합 규칙 없음 + "no reviewer" 품질보증이 pyreez에 의존하나 부재 처리 없음 → 핵심 산출물과 핵심 품질주장 둘 다 홀 |
| reflect | 🔴 중대 | "substantive" 보증이 미정의 — hook은 *존재만* 검사(게다가 4섹션 중 3만, warn-only). hollow지만 구조완전한 Reflect가 통과 |

### 🔑 핵심: 같은 홀이 스텝 전반에 *반복*된다 (개별 아닌 패턴 결함)
스텝마다 따로 난 게 아니라 **8개 구조 패턴**이 반복 — 패턴을 잡으면 다수가 닫힌다:

| # | 반복 패턴 | 해당 스텝 |
|---|---|---|
| **P1** | **성공/주요 출력 미정의** (실패·특정 경로만 정의) | verify(PASS 없음), report(fail/escalate 없음), test(GREEN 분기 없음) |
| **P2** | **자기 주요 도구 부재 무방어** (그 스텝의 핵심 tool이 죽으면?) | ground(ED MCP), implement(firebat/emberdeck), verify(pyreez/firebat) |
| **P3** | **입력 exactly-one-of(Spec\|Decide)인데 `based_on`은 한 모양 하드코딩** → Spec-less Decide 체인에서 provenance 깨짐 | test(`{spec_ref}`만), implement(`{spec_ref,test_ref}` 무조건) |
| **P4** | **정체성이 Decide(Design)에 결박됐는데 Plan/Record로도 실행됨** | spec(Design 전용 가정), decide(금지된 Investigate 옵션 읽음) |
| **P5** | **품질보증이 정의·강제자 없이 선언만** | reflect("substantive"), verify("no reviewer") |
| **P6** | **출력 enum 무정의** | triage(flow_type `...`/complexity_signal), report(severity/priority), spec(measurement/edge_cases), ground(unknowns.dim) |
| **P7** | **빈/degenerate upstream → 조용히 proceed 고무도장** | investigate(빈 Ground→force proceed), decide(N=0), spec/test/implement(빈 AC/files) |
| **P8** | **garbage-in 맹신** — upstream이 항상 깨끗·존재·정형이라 가정, detect/escalate 없음 | 거의 전 스텝 |

### 적대검증 교정
- **triage**: 평가자 "경미" → 적대 **"중대"로 상향**(필수 미생산 필드 + 미정의 safety 판별자는 cosmetic 아님). 채택.
- **ground**: 적대가 worst-hole(ED 총부재) **과장 지적** — capture 실패는 unknowns로 매핑되므로 "전혀 무처리"는 거짓. 단 ED *총부재 vs referent-부재* 구분 부재는 잔존 → 경미 유지.
- **investigate**: Compound no-op 누락은 *부분 오탐*(다른 경로로 일부 커버). 경미 유지.

### 정직한 한계 (이 패스)
- 이건 *prose 계약 설계* 검토다 — 런타임 미검(하네스 미구현). 홀 = "계약이 침묵/모순/미정의"지 "반드시 런타임 실패"는 아님(상당수는 orchestrator가 암묵 처리할 여지). 그러나 *계약이 강제 못 하면 보장도 없다*가 §16 입장.
- **다음 단계는 이 홀들의 보강 설계** — 단 P1~P8 패턴 우선(개별 스텝 땜질 아님). 아직 설계 안 함(이 문서는 *문제 식별*까지).

## P1~P8 패턴 보강 *설계* (16 agent, 적대검증 교정 반영 — 2026-05-31)

> 원칙: **기존 메커니즘 재사용**(새 subsystem 발명 0) + **가장 단순한 정답** + **경계 안전**. 적대검증 결과: **P4만 그대로 견고(✅), 나머지 7개는 교정 필요(🟠), 기각 0.** 코드·스키마 아님 — *prose 계약을 어떻게 고칠지*의 설계.

### 🔑 교정이 드러낸 3개 횡단 원칙 (경계 플래그 대부분을 해소)
보강안들이 처음에 경계를 넘은 이유가 *같았다* → 3원칙으로 통일하면 깨끗해진다:
1. **도구 부재 라우팅 = 도구 역할로 갈림.** *주요* 도구 부재(그 일을 못 함) → **escalate**. *보조/enhancement* 도구 부재 → **degraded_pass**. (ED=Ground 주요→escalate, firebat=Implement/Verify 게이트→escalate, pyreez=Verify enhancement→degraded.) → P2의 자기모순 해소(unknown-disposition.md L24 `tool_unavailable→escalate`와 일치).
2. **제어 신호 소유권.** `request_upstream_deepen`은 **Decide 전용**(decide/README L99-103, cap "Decide→Ground/Investigate"). 다른 소비자는 *기존 `failure_origin` escalate*로 라우팅 — 새 권한 부여 금지. → P7 경계 해소.
3. **"정당하게 빔"(verdict) vs "결손/기형"(mechanical error) 구분.** 빈 입력이 *합법적 결과*면 verdict(no_op/proceed), *upstream 결손*이면 mechanical error→escalate. → P7/P8을 하나로 묶음.

### 패턴별 보강안 (교정 후)
| 패턴 | 판정 | 보강 (재사용 메커니즘) | 교정 핵심 |
|---|:--:|---|---|
| **P1** 성공출력 미정의 | 🟠 | 각 스텝이 **success 분기 포함 전체 result enum** 선언 — Investigate 5-state + 라우팅 테이블 재사용. verify=PASS 객체+집계규칙(any-fail→fail/all-pass→pass), test=non-RED 분기(unexpected_green/no_op), report=terminal 성공객체 | **report에 Investigate식 gating 주지 말 것**(over-reach). goal-backward 기록은 *전 분기* 유지(fail에서 빼지 말 것). **schema 레이어가 이미 shape는 함** → 이건 prose 백포트 |
| **P2** 도구부재 무방어 | 🟠 | 주요활동 도구에 tool_status 분기(present→정상 / absent→정의된 verdict). M3 degrade + R12 failure_modes 재사용 | **원칙①로 통일**: ED/firebat=escalate, pyreez=degraded. degraded_pass를 ED에 쓰지 말 것(기존 matrix와 충돌) |
| **P3** based_on 하드코딩 | 🟠 | `based_on`을 입력계약(min-1-of Spec\|Decide, Test 조건부)에 **맞춰 조건부 선언**. 이미 명시된 input-contract 규칙 재사용 | test/implement 비대칭 통일. (ref *해소* 검증은 M2 — 별개) |
| **P4** Design 결박 | ✅ | Spec/Decide를 **3 mode 전체에 일반화** — Spec은 Decide가 낸 무엇(design_doc/option/decision)에서든 AC 추출, intent-card 링크는 Design 조건부. Decide 비강제 tiebreak는 **R6의 실재 필드**(issues count/risk/affected_files) 읽음(금지된 "options" 아님) | 그대로 — 새 기계 0, 경계 안전, 두 스텝만 정확히 닫음 |
| **P5** 품질보증 무정의 | 🟠 | **verify 측은 완전 견고**: "no reviewer"를 "LLM판정+cross-verify 완화"로 정직 강등, pyreez 부재=degraded 분기(원칙①), pyreez 불일치=failure_origin=verify escalate | **reflect 측 over-eng**: "artifact ≥1 인용 강제" 빼라(provenance는 Investigate/Ground 일 + cross-flow 패턴이 못 댐=새 홀). reflect 보강 = hook을 4섹션 검사로 고치고+non-filler 체크+"substantive 보증"을 정직 강등 |
| **P6** enum 무정의 | 🟠 | 열린 enum을 **canonical source로 닫기**: triage flow_type=16 정규 id, complexity_signal=Comprehend 생산규칙, report severity/priority=Investigate risk 척도(단일), spec measurement=verify_probe | items 1-3 그대로. **item5(ground unknowns.dim)**: dimension *사실 태그*만 열거, Investigate의 disposition 토큰 발명 금지(분류는 Investigate 일). §10 research/audit 별칭 정합 플래그 |
| **P7** 빈 upstream 고무도장 | 🟠 | 소비자마다 "degenerate upstream=감지된 precondition fault" 1절 + Investigate가 **빈 entry_nodes로 V1(빈 issues→proceed) 차단**. request_upstream_deepen+no_op+escalate 사다리 | **원칙②**: 비-Decide 소비자는 request_upstream_deepen 못 씀→failure_origin escalate. **원칙③**: 빈-합법 vs 빈-결손 구분 |
| **P8** garbage-in 맹신 | 🟠 | **단일 횡단 input-precondition 절**(전 소비자 동일 문구): 필수 upstream 필드 *존재+정형* assert(진실 아님—그건 Verify), 결손/기형→escalate(failure_origin=upstream). Ground의 active_flow_state mechanical-error 패턴 일반화 | 형태 견고·over-eng 아님. ping-pong은 기존 5-누적-fail halt cap이 이미 bound. 잔여: 정형이나 *의미상 틀림*은 못 잡음(Verify/환원불가) |

### 정직한 한계 (이 보강안)
- 전부 **M1 shape/계약 수준** — 닫는 건 "계약이 침묵/모순/over-reach"지 *self-asserted truth*가 아님. verify `goal_satisfied:true`, test `unexpected_green`, tool_status `present`는 여전히 LLM 자기단언(환원불가 floor, cross-verify로 *완화*만).
- **schema 레이어(옆으로 둠)가 P1 성공분기·P6 enum 일부를 이미 shape로 실현** → prose 보강과 schema가 *언젠가 정합* 필요(지금은 schema 안 건드림).
- 이건 *보강 설계*까지 — 실제 README 편집(계약 반영)은 별도 단계. 네 확인 후 진행.

## 결함 없는 스텝 계약 설계 완료 (21+3 agent, 2026-05-31)

> 위 검토(0완전/8중대/2경미)에서 찾은 *각 스텝의 모든 홀*을 P1~P8 + 3원칙으로 닫은 **결함 없는 successor 계약**을 `harness/steps/<step>.md` 10개로 작성. 각 fix는 `(P#/principle)` 태그로 추적 가능. legacy/steps = 홀 있는 원본, harness/steps = 교정본.

### 검증 결과 — 10/10 모든 실제 홀 닫힘
| | 결과 |
|---|---|
| 모든 실제 홀 닫힘 | **10/10** |
| 경계 위반 | **0** |
| over-engineering | **0** |
| 원의도 충실 | **10/10** |
| 남은 "새홀" | 전부 cosmetic (ordering nit / stray anchor 오타 / prose↔schema 드리프트 플래그) — 재오픈된 실제 홀 0 |
| critic 횡단 | **systemic 원칙 드리프트 0**, minor 불일치 3 |

각 스텝이 새로 갖춘 것: ① success 분기 포함 전체 result enum + 라우팅 테이블, ② Failure & degrade handling(도구부재 principle 1), ③ Input preconditions(필수 upstream 존재+정형 assert→escalate, 5-누적-fail cap이 ping-pong bound), ④ 모든 열린 enum/생산규칙/미정의 필드 닫음. **새 subsystem 0 — 전부 기존 메커니즘 재사용.**

### 적용한 cheap 결함 수정 (검증 후)
- `decide.md` scope_ref 주석의 모호한 "§5" → "DECISIONS §5 (Postgres row id)"로 명확화 (in-file anchor 오해 제거).
- `verify.md` escalation `failure_origin` annotation `verify|cap_exceeded` → `verify|ground|cap_exceeded` (stale_2nd_attempt 경로가 ground origin 사용 — annotation이 실제보다 좁았음).

### 남은 minor (재오픈 홀 아님 — 기록만)
- **no_op `suggested_action` 어휘 미통일**: investigate `{abandon|wait_for_change|reframe_request}`, test `{abandon|reframe_request}`, report `{none|reframe_request}`. 각자 로컬 정당(test엔 wait-for-change no_op flow 없음 등) — 미정의 값 0이라 결함 아니나 단일 canonical enum 부재. 통일 강제는 *인위적*이라 보류.
- **prose↔schema 드리프트**: spec `investigate_ref` 조건부(prose) vs hard-required(stale schema), ground 신규 `ground_result`/`omitted_fields`가 schema 미반영. schema 옆으로 둔 상태 → *후속 정합* 플래그(지금 schema 안 건드림).

### 정직한 한계
- **M1/계약 수준** — 닫는 건 "계약 침묵/모순/over-reach"지 self-asserted truth(forged tool_status, LLM goal_satisfied)가 아님(§16 환원불가 floor, cross-verify로 완화만).
- 이건 *설계*까지 — 이 계약을 *동작 하네스로 배선*하는 건 별도 단계.
...
## 스텝별 structured_output 스키마 설계 (22 agent 설계+적대 + 기계 검증)

> R-rule disposition을 *실제 JSON Schema*로 구현. 산출물: `harness/schemas/` — `_defs.schema.json`(공유 $defs 30개) + 10개 `<step>.schema.json`. draft 2020-12, grammar-constrained `structured_output` 대상. **이 레이어는 M1(SHAPE)만** — M2/M3 sibling을 *명명*하되 구현 안 함(하네스 미구현).

### 무엇을 실현했나
- **M1 grammar**: discriminated union (triage 4-type / decide 3-mode / investigate 5-state compat / verify 5-result / report 6-type) = oneOf+discriminant, required[], enum.
- **R23 count-type** `CountClaim {value, source:{command, raw_stdout_sha256}}` — measured count에 bare integer *생성 불가*.
- **M3 degrade** `DegradableMeasurement = oneOf(Measured | Omitted{reason,source_tool})` — null/placeholder *생성 불가* (R22). Ground volatile, Verify mechanical에 적용.
- **M2 contracts** = `x-validator-contract` annotation (grammar가 truth 강제하는 *척 안 함*; orchestrator 코드 체크를 문서화): CountClaim 재파싱, VerifyProbe 재실행, Sha256 재해시, SourceManifest cited-line, ExecMeta pid≠pid, DeclaredNextStep R16 체인, ParallelMarker same-file.
- **floor**: `UnverifiedFlag`(KEEP 극성, forbid 없음), `source_tool` provenance, `CrossVerifyFlag`(R5).
- **§5**: 모든 `*_ref` = `RowRef`(Postgres row id). 유일 파일 예외 = Reflect Tier2 `.claude/rules/*.md`.
- **M4**: `_defs`가 primitive 단일 source — 10 스키마가 drift 불가.

### 적대 검증이 잡은 실제 grammar 버그 3개 (전부 수정+재검증)
1. `implement.based_on` `oneOf`("min-1 of Spec|Decide") → Decide-then-Spec가 **둘 다** 가지면 생성불가 → `anyOf`로 수정.
2. `report` top `additionalProperties:false` → spike/plan_standalone/compound 분기 **unsatisfiable** → `unevaluatedProperties:false`로 수정.
3. `verify.internal_passes` if/then이 required만 추가하고 반대 변종 **금지 안 함** → 각 flow_kind 분기가 wrong 변종 forbid.
+ `investigate.CompatibilityIssue.source_tool` → required 승격(R26 floor 실 강제), `ground` + `source_manifest`(R36 실현).

### 기계 검증 결과
11/11 valid draft-2020 · 깨진 ref 0 · M4 drift 0 · §5 위반 0 · **만족가능성**: 전 스키마 + 전 discriminant 분기(report 6/6, verify flow_kind 2/2, triage 4/4)에 valid 인스턴스 생성 확인.

### ⚠ 플래그 (사용자 결정 필요 — §10 소유)
triage 스키마의 `FlowType` enum = Signal Table 16개(snake_case)인데 **DECISIONS §10 flows.flow_type 예시는 `research`/`audit`를 씀** (Signal Table엔 없음 — research≈exploration/plan-standalone, audit≈review로 추정). §10이 illustrative이고 Signal Table이 canonical이라 봤으나, §10 UI 예시가 `research`를 live로 사용 → **enum 정합 reconcile 필요** (스키마를 16으로 둘지, §10 별칭을 흡수할지).

## 정직한 한계
- 이건 *설계 문서* 검토다 — *런타임 동작*은 미검(하네스 미구현).
- 스텝 스키마는 **M1만** — `x-validator-contract`(M2 truth 체크)는 *설계 annotation*이고 validator 코드는 미구현.
