...
- *Compound 전체의 no_op*은 **sub_flow_verdicts의 result가 전부 no_op일 때에만** 성립한다. 하나라도 no_op이 아니면 Compound 전체는 no_op이 아니며, partial한 의미를 가지므로 `partial_proceed`(일부 sub-flow proceed) 또는 `proceed`로 라우팅된다. (이 규칙이 legacy의 누락된 Compound 행을 닫는다.)
...
| `partial_proceed` | **부분 SUCCESS 분기.** 일부 scope accept + 일부 reject | `partial_scope_handling{proceed_set, blocked_set, followup_required}` | Decide 진입 — proceed_set 처리, blocked_set은 followup_flows로 큐잉 |
| `blocked` | blocking issue 또는 escalate-disposition 존재 | `blockers: [issue_id]` (비어있지 않음) | **Flow halt 강제** — Decide 미실행, blockers surface |
| `needs_clarification` | user/caller 응답 필요 | `open_questions: [issue_id]` (비어있지 않음) | **Decide 미실행** — NEEDS_CONTEXT → 응답 후 Investigate 재invoke |
...
- compatibility_verdict 명시 (V1-V13 통과; V1은 PC1 전제 하에서만 발화)
...
- **triage_mismatch 일관성** (신규): emit 시 production rule 충족(구조적 모순 evidence 있음) + 동반 result
  ...
  미사용(principle 2)
- **Compound aggregate no_op 일관성** (신규): Compound가 no_op이면 sub_flow_verdicts result가 *전부* no_op인지 검증
...
V4.  V2/V3 모두 미충족 (+ no_op 아님) → result=proceed
...
V13. Partial-proceed result: 일부 dependents accept + 일부 reject 시 result=partial_proceed.
     issues는 scope.target_set per 영향 범위 분리. partial_scope_handling{proceed_set, blocked_set, followup_required}.
