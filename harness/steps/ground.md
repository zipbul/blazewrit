...
| `channel` ∈ {user_session, a2a, ci} | enum 밖 값 = escalate, `failure_origin=ground` |
...
2. Volatile Capture (flow_type별 선언된 profile — Ground 본 contract가 authoritative)
   ...
   - 명령 부재 시: M3 Omitted{reason='skipped', source_tool} (skipped-with-reason carrier)
...
| `escalate` | **PRIMARY 결손 또는 mechanical error** — ED MCP 부재/error/timeout, input precondition 위반, malformed active_flow_state. Ground가 *자기 일을 못 함* | `escalate_detail: { failure_origin: ground, reason, evidence }` (정상 [Output] 미산출 또는 부분) | **Flow halt** → `failure_origin=ground` 라우팅 (producer⇄reviewer 재진입, 5-fail halt cap) |

**분기 선택 규칙 (mechanical)**:
1. input precondition 위반 → `escalate`.
2. ED MCP 부재/error/timeout (primary tool down) → `escalate` (principle 1: ED는 PRIMARY → escalate, NOT degraded; unknown-disposition.md L24 `tool_unavailable→escalate`와 정합). (P2 + principle 1)
3. malformed active_flow_state → `escalate`.
4. 위 셋 다 아니고, conditional volatile 결손/racing 잔존만 있음 → `degraded_pass`.
5. 결손 없음 → `ok`.
...
volatile_state:
  ...
  typecheck: <DegradableMeasurement>              # measured{status: success|fail, output_hash, source_command, captured_at, duration_ms} | omitted{reason: skipped|timeout|tool_failed, source_tool}
  ...
  git: { branch, dirty, head_start, head_end, recent_commits: [...] }   # non-degradable factual state (always measured)
...
| `ed_drift` | card↔code 불일치 (ED가 보고) | `card_code_mismatch` |
...
| 검사 | 기준 |
|---|---|
| ground_result | `status` ∈ {ok, degraded_pass, escalate}. `degraded_pass`면 `degrade_reasons` 비어있지 않음. `escalate`면 `escalate_detail.{failure_origin, reason, evidence}` 존재 |
...
| volatile_state | profile-required 각 명령에 explicit M3 분기 (measured{status} OR omitted{reason}). omitted면 reason ∈ {skipped, timeout, tool_failed} |
...
| freshness | `task_subgraph.status=measured`면 `ed_snapshot_version` 기록. `git_head_start` 항상 기록 |
...
Ground는 conflict resolution을 하지 않는다 (orchestrator 책임). Ground는 *해결 후 잔재* 인지 + *기형 입력* escalate만 한다.
...
3. **사실 surface**: `unknowns[{dim:referent, reason:"unrecognized flow_type — universal profile applied", source_tool}]` 1개 추가하여 *Ground가 profile을 추론하지 않았음*을 명시.
