...
|---|---|---|
| `red` | 1개 이상 test 작성·실행, 결과는 모두/일부 RED (적어도 하나 `status: RED`) | **정상 — Test⇄Implement 루프 진입** (Implement가 RED를 GREEN으로). RED인 test들이 turn-green 대상. |
| `unexpected_green` | test 작성·실행했으나 **전부** `status: unexpected_green` (작성된 RED 의도 test가 하나도 실패하지 않음) | **정상 발생** — Implement에 turn-green 대상 없음. **Verify에게 신호** (failure 아님, escalate 아님). orchestrator는 Test⇄Implement 루프를 skip하고 Verify로 진행. Verify가 "목표가 실제 충족됐는가/재현 불가가 맞는가"를 판정. P0 retroactive에서 이 분기가 *기대값*. |
| `no_op` | **작성할 test가 정당하게 0개** (아래 "빈 AC" 정당-빔 케이스). escalate가 아니라 verdict. | `no_op_details` 동반. orchestrator는 Reflect 실행 (abandonment 분류). |
...
```yaml
test_result:
  result: red | unexpected_green | no_op        # P1: 전체 enum (success 분기 포함), status:RED 하드코딩 폐지
  schema_version: 1

  tests_added:                                   # result ∈ {red, unexpected_green}일 때 비어있지 않음
    - file_path
      test_name
  ...
  profile?:                                       # Performance — 측정 가능하도록 단위/정의 명시 (reviewer 요구)
  ...
  migration_validation?:                          # Migration
    script_path
    dry_run_result: pass | fail | error          # 구조화 enum (이전엔 무정의)
    dry_run_detail?                              # error/fail 시 진단 텍스트
...
> `?` 표기는 "optional (flow-conditional)" 의미 (프로젝트 schema 규약). result=red|unexpected_green이면 `tests_added`는 비어있지 않다; result=no_op이면 `tests_added` 생략·`no_op_details` 필수.
...
**빈/degenerate 입력** (P7 / principle 3: 정당-빔 vs 결손 구분):
- `acceptance_criteria: []` (well-formed empty list) — **두 케이스 구분**:
...
- **result enum 정합** — 적어도 하나 RED면 `result=red`; 전부 unexpected_green이면 `result=unexpected_green`; test 0개면 `result=no_op`+`no_op_details`
...
- Performance: `profile.metric`+`unit`+`baseline.measurement_method`로 *측정 가능*
- Migration: `dry_run_result` enum 정합
