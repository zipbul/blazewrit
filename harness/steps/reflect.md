...
```
reflect_result:
  result: completed_reflection | reflect_incomplete | skipped_suspended | precondition_escalated | archive_escalated
  ...
  tier2_writes?: [ { topic, action: append | create | deferred, rule_ref } ]   # Dedup/Promotion 결과 (deferred = Tier 2 write 경합/IO 실패로 Tier 1에 pending 보존; primary 학습은 보존됨 — 아래 write 경합 처리)
  incomplete_details?: { failing_sections, iterations_used, reason }   # result=reflect_incomplete일 때 필수
  escalate?: { failure_origin, reason }      # result ∈ {precondition_escalated, archive_escalated}일 때 필수. failure_origin = cap_exceeded (post-flow halt origin — 아래 참조)
```
...
| Tier 2 file write 충돌/IO 실패 | degraded — Tier 1에 pending 기록, `tier2_writes[].action=deferred` 표기, escalate 아님 | principle 1 (Tier2=enhancement) |
