...
| **L1 에이전트** | per-intent 플래너 | **자기 의도의** pending/held 잡 — 프로젝트가 달라도: amend/split/add/비순환 edge 추가 (예산 내). 타 의도 잡에는 **제안만**; 대상 의도 플래너가 자기 예산·스코프 내 자율 수락 |
| **L2 인간** | 승인함 | remove/reorder/비가역 cancel, 예산 소진, 에스컬레이션, 충돌 미해결. **일괄 승인**: 하나의 게이트가 토폴로지+cancel+재배선 패키지를 한 번에 |
...
- **왕복 횟수 제한 없음** — 내용이 바뀌며 오가는 것은 조율이다. 벽은 기계적 사실에만: ① 동일 요청 반복(잡+변이+내용해시) = 루프 → 차단 ② 의도당 변이 예산 ③ **무진전 감지**(같은 충돌이 내용 변화 없이 재발) → 에스컬레이션.
- stale 감지: 제안이 참조한 generation이 지나가면 자동 무효.
...
협상 FSM · 글로벌 최적 스케줄러 · partial-done 상태 · 완전 이벤트소싱(로그는 감사용) · 왕복 상한 · preempt/kill.
...
4. **P4** 변이 연산 + 조율 프로토콜 + 감쇠(동일해시·예산·무진전) + supersede
