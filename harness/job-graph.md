# Task / Job Graph (v4 — 합의 수렴본, 확인 라운드 중)

> Status: **합의 수렴.** 2라운드 4자 시나리오 소진 + 과설계 감사(grok·codex·서브에이전트·자체). v2(표현력 부족) → v3(뿌리 정정) → v4(최소·정확). 원칙: **추가 요소는 구체 BROKEN 시나리오 하나 이상을 닫아야 하고, 다른 원시로 표현되면 넣지 않는다.** 확인 라운드에서 3자 사인 후 동결.
...
1. **write 권한 강제**: 앱 write 경로가 `jobs.project_id == 요청 프로젝트` 검사(또는 RLS). "강제"는 단어가 아니라 이 경로.
2. **seal = insert-freeze**: `task.status != 'open'`이면 그 태스크에 잡 INSERT 거부(트리거/CHECK). 재실행은 제자리 gen++라 INSERT가 아니므로 봉인과 충돌 안 함. 봉인 태스크에 새 잡이 정말 필요하면 명시적 re-open 전이.
3. **done 원자성**: `sealed ∧ 모든 잡 terminal` 확인 → `done` 전이를 같은 트랜잭션에서. insert-freeze가 그 사이 잡 추가를 막아 race 봉합.
4. **liveness backstop**: task-타깃/external dep가 안 풀리면 **정체 타이머가 대기자 에이전트를 깨워** 결정(재협상/부재수용/에스컬레이트). deadline auto-release 없음(승인게이트 자동충족 위험). 침묵 hang 금지.
5. **stale 해소**: dep `stale`(expected_gen 불일치) → 대기자 에이전트 wake해 새 gen 대상 재선언.
6. **task-타깃 outcome 유도**: target='task'이면 task.status → member outcome (done→satisfied, cancelled→cancelled, 소속 잡 일부 failed→failed). 규칙 고정.
...
1. **P1** 최소 스키마(tasks/jobs/deps/dep_members/external_gates) + 상태기계 + 봉합규칙 1·2·3·6·7 + cycle·ready 순수함수 (TDD; 단일 잡·단일 dep = 현행 보존)
