...
**금지된 출력 (Investigate-Reviewer가 regex 검출)**:
- 미래 artifact path mention: `.blazewrit/plans/.+-plan\.md`, `.blazewrit/plans/.+-decide\.md`, `.blazewrit/reports/`, `.blazewrit/spec/` — Investigate는 *현재 상태 해석*만, 다음 step output 어디 살지 모름
- option/design verb: `chosen`, `choose`, `select`, `pick`, `recommend`, `design`, `proposed approach`, `architecture (as new)` — 모두 Decide 영역. *risk identification ("omission risk")*는 OK, *suggestion ("layered approach")*은 NOT OK.
- *validity_check* 섹션은 "task가 의미 있나?"만 — 결과 path / artifact format / downstream consumer 언급 금지.

## Fact Verification (R13 강제)

모든 출력 필드 — 특히 invent-prone:
- `compatibility_verdict.source_version.{ed_snapshot_version, rules_version, contracts_version}`: tool 부재 시 *필드 omit* (placeholder "not_tracked" 등 금지). emberdeck 없으면 `ed_snapshot_version` 자체 omit.
- `architecture_impact.new_modules`, `public_api_changes`: ED graph + impact ripple에서 derive — 없으면 빈 list `[]`. invent 금지.
- `compatibility_verdict.result` enum 검증: `proceed | blocked | needs_clarification | no_op | partial_proceed` 외 값 emit 금지.
