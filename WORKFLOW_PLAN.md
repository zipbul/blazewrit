...
```
None (자유 대화/논의) ↔ Triage → Flow[Ground → Analyze → 기획? → Spec? → Core Steps → Verify → Reflect]
```
...
- **Analyze**: **Task-specific impact analysis** — Ground 산출물 위에서. 코드 분석, 의존성 매핑, 영향 범위, 제약 조건. *이해/사실 캡처는 Ground 책임* — Analyze는 *해석*. 깊이는 플로우 정의가 결정 (Feature=깊음, P0=최소, Chore=최소).
...
| Analyze | **Task-specific impact analysis** — Ground 위에서 영향 범위·옵션 후보·위험 도출. 사실 *해석*. *이해 자체는 Ground*. 깊이는 플로우 정의가 결정. |
...
| Flow | Universal + 추가 |
|---|---|
| Feature, Bug Fix, Bug Fix P0, Refactor, Test, Chore, Review, Retro, Exploration, Compound, 기획-standalone, Spike | Universal만 |
| Performance | + perf baseline 측정 (벤치 명령 실행) |
| Migration | + dependency/compatibility audit |
| Bug Fix Unreproducible | + observability data fetch |
| Release | + version·changelog 상태 |
...
### Active Flow Conflict 우선순위 (충돌 해결 자체는 orchestrator 책임)

Ground는 conflict resolution 안 함. 단 `active_flow_state`가 새 `request_text`와 관련될 때:

| 상황 | Ground 처리 |
|---|---|
| 새 request가 active flow와 무관 | 그대로 진행 (orchestrator가 이미 해결한 상태로 가정) |
| 새 request가 active flow와 관련 (같은 영역) | unknowns에 `active_flow_overlap` 명시, 진행 |
| `active_flow_state` 없음 | 그대로 진행 |
...
| 항목 | 책임 |
|---|---|
| Feasibility 판정 (proceed/blocked) | Analyze |
| 영향 범위 *해석* | Analyze |
| 옵션 후보 / 접근 결정 | 기획 |
| 카드 *생성* (intent/spec) | 기획/Spec — Ground는 *읽기*만 |
| 측정값 *의미 판단* (예: "이건 느림") | Analyze |
| 위험·심각도 *판단* | Analyze / Verify |
| 코드 변경 | Implement |
...
모든 flow가 **Ground**로 시작. `Ground(volatile_profile)`은 flow별 선언된 measurement profile (universal + conditional).  
Analyze는 *task-specific 해석/영향 분석* (사실 캡처는 Ground 책임이므로 괄호 안 내용은 *분석 초점*).
...
Release:
  ...
  → Analyze(minimal: version, CI status)
...
Compound:
  ...
  (각 Sub-Flow는 자체 Ground 실행 — 해당 sub-flow type의 volatile_profile 적용)
```
