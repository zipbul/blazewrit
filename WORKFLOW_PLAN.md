...
### R16. Chain Enforcement (Cross-Model audit C-1)

Codex round 4 검증에서 발견: Decide/Report가 *flow chain definition* 위반하는 "next step" 주장 (예: plan-standalone chain은 `D→Report→Verify→Reflect`인데 Decide/Plan이 "Next Step = Spec" 주장). 이는 **release-blocking architectural defect** — production planner가 자기 정의 위반.

**Mechanical rule (reviewer hook)**:
- 모든 step output이 "next step / next_step / Next Step / 다음 step" 언급할 때 → flow definition의 `flow_chain[current_step_idx + 1]` 과 *정확히 일치* 해야 함
- Unmatch 시 reviewer FAIL `reason: "R16 chain violation — declared next_step=<X> but flow_def says next=<Y>"`
- 단 *followup_flows*는 별도 — chain에 없는 flow type을 *queue*하는 건 OK (자동 실행 아님, 별도 flow_id)

**구현**: orchestrator가 reviewer invocation 전 flow_def 주입 → reviewer prompt에 "expected_next_step: <Y>"  포함 → reviewer가 producer artifact에서 next_step 추출 + 비교.

**Decide(Design) chosen_architecture의 "downstream chain" 언급**: 반드시 *flow chain definition의 verbatim*. 다른 chain 주장 = R16 FAIL.

### R17. Fact Re-Verification (Codex audit C-2, C-3)

Codex round 4에서 발견: Ground이 emit한 `.gitignore exclusion claim` (잘못된 사실), `git HEAD` (stale) 등을 *downstream agent*가 *재검증 없이* propagate. 즉 *fact verification protocol*은 *Ground emission 시점*에만 적용 — *downstream consumption 시점* 재검증 없음.

**Verify mechanical re-execute** (코드 flow + 비코드 flow 공통):
- Ground.volatile_state의 모든 Bash command를 *Verify가 재실행* → 결과 비교
- Ground.task_subgraph의 sha256 hash를 *Verify가 재계산* → 변경 detect
- mismatch 발견 시 → `failure_origin: ground` (fact emission이 잘못됐거나 stale) + `RETRY_EXHAUSTED` 처리

**Stale detection**:
- `git rev-parse HEAD` 변경 시 (Ground capture 시점 vs Verify 실행 시점) → flow의 fact-base가 stale → re-Ground 자동 트리거 (R11 cycle cap 적용)
- 비-git fact (예: package.json content): Ground 시점 sha256 vs Verify 시점 sha256 비교

**Investigate/Decide 단계 mechanical fact re-check** (선택, 비용 trade-off):
- 큰 fact (gitignore, package.json) 는 매 step 재읽기 비용 부담 → Verify에서만 재검증
- 작은 fact (file 존재 여부, 단순 count): 매 step 재check 권장

### R18. Semantic Boundary Hardening (Codex audit C-4, C-5)

Codex가 regex로 못 잡은 *semantic boundary leak* 발견. 추가 mechanical 정책:

**Ground god_node 정책**:
- ED MCP 사용 가능 → `query_graph` degree 기준으로 god_node 분류 (mechanical)
- ED 부재 시 → `god_nodes` 필드 **omit** (substitute 금지 — "cross-reference count" 같은 자체 분류는 interpretation = boundary 위반)
- Ground-Reviewer hook: `god_nodes` 필드가 emit됐는데 `ed_query` unknown이면 FAIL `reason: "R18 god_node interpretation when ED absent"`

**Ground conflicts derived statement detection**:
- conflicts 섹션 내 *derived statement patterns* 검출 (regex 못 잡는 semantic):
  - `"No <X> exists"`, `"<X> is missing"`, `"<X> not found"`, `"count(<X>) = <N>"`, `"<X> contains <N>"` — *결론* 또는 *derived counting*은 interpretation
  - 정답: raw inclusion만 — `"AGENTS.md:7 contains: '<exact quote>'"`. 결론·counting 없음.
- Ground-Reviewer hook 강화: conflicts 섹션 LLM judgment check (regex로 못 잡으면 reviewer가 semantic 평가)

**Investigate conclusion creep**:
- Investigate가 `compatibility_verdict.reason`에서 "Ground supplies sufficient referent set" 같은 *meta-conclusion* emit 시 — 결론 아닌 *fact list*만 (`"Ground provides: <list of facts>"`)
- Investigate-Reviewer hook: meta-conclusion verb detect (`supplies`, `provides sufficient`, `enables`) → FAIL

### R19. Concrete Task List Mandate (Codex audit C-7 — industry gap)

Codex가 spec-kit/GSD-2/MetaGPT/Devin과 비교 — blazewrit는 *requirements*는 있지만 *concrete executable task list* 없음. Industry-standard 부족.

**Decide(Design) output 강제 추가 필드**:

```yaml
task_list:                              # NEW required field for Design mode
  - id: T1
    description: <imperative action>     # "Write section 1.1 about X"
    inputs: [<artifact ref>]             # required (Spec/Implement 호출 시)
    outputs: [<artifact path or section ref>]  # required
    depends_on: [<task_id>]              # ordering
    acceptance_test:                     # R20 link
      type: file_exists | grep | command | sha256_match | line_count_range
      target: <path or expression>
      expected: <value or pattern>
    verify_probe: <bash command>         # Verify가 mechanical 실행
    est_effort: trivial | small | medium | large
```

**Decide-Reviewer 추가 check**:
- Design mode → task_list.length ≥ 1 필수
- 각 task에 acceptance_test + verify_probe 명시
- 누락 시 FAIL `reason: "R19 missing concrete task list / probe"`

**Spec/Implement이 task_list를 직접 consume** — 더 이상 "Spec이 finalize" 같은 deferral 금지.

### R20. Verify Probe Mandate (Codex audit C-8)

Codex 발견: Plan에 acceptance requirements는 있지만 *concrete verification command* 없음 → Spec에 위임 → Spec은 flow chain에 없음 → 결국 unresolved.

**모든 acceptance requirement에 verify_probe 강제**:

```yaml
requirements:
  - id: REQ-1
    description: <what must be true>
    verify_probe:                        # NEW required
      type: file_exists | grep | command | sha256 | http_get | line_count
      target: <concrete path/url/command>
      expected_result: <pass condition>
      negative_test?: <fail condition>   # optional but recommended
```

**Verify가 mechanical execute**:
- 모든 requirement.verify_probe 실행
- Pass 조건 매치 시 → requirement PASS
- 실패 시 → `failure_origin: <emitting step>` + 어느 REQ 실패 명시
